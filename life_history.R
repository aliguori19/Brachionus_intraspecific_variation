...
# Multipanel reproduction (by cohort) plot---------------------

tiff("rep cohort.tiff", units="in", width=10, height=12, res=300)
...
rep_column <- ggarrange(rep_Bp_plot, rep_RUS_plot,
...
rep_column <- annotate_figure(rep_column, bottom = textGrob("Cohort", gp = gpar(fontsize = 24)), left = textGrob("Reproductive period (% of lifespan)", 
                                                                                                   rot = 90, gp = gpar(fontsize = 24)))
big_boi <- ggarrange(LRO_summary, rep_column,
                     ncol = 2, nrow = 1)
big_boi
dev.off()


# Multipanel reproduction (by age) figure------------------------

tiff("rep_by_age.tiff", units="in", width=11, height=12, res=300)
...
big_boi2 <- ggarrange(RO_curves, int_value_combo,
                     ncol = 2, nrow = 1)
