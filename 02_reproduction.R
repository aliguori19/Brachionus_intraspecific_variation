...
balance <- LRO_per_ind_F1 %>% count(maternal_age, strain)
...
tiff("LRO.tiff", units="in", width=12, height=12, res=300)
...
RO_curves <- annotate_figure(RO_curves, bottom = textGrob("Age (d)", gp = gpar(fontsize = 24)), left = textGrob(expression(paste("Offspring ind"^"-1"*"day"^"-1")), 
...
LRO_summary <- annotate_figure(LRO_summary, bottom = textGrob("Cohort", gp = gpar(fontsize = 24)), left = textGrob(expression(paste("Lifetime reproductive output (offspring ind"^"-1"*")")), 
                                                            rot = 90, gp = gpar(fontsize = 24)))
