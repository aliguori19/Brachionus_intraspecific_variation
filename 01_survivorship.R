...
# Y vs. M cohort:
surv_L5_M <- filter(surv_L5, maternal_age != "10") # excluding 0 cohort
...
# Y vs. O cohort:
surv_L5_O <- filter(surv_L5, maternal_age != "6") # excluding M cohort
...
# Y vs. M cohort:
surv_RUS_RE_M <- filter(surv_RUS_RE, maternal_age != "10") # excluding O cohort

RUS_RE_M <- survdiff(Surv(surv_time, censor) ~ maternal_age, data = surv_RUS_RE_M)
1 - pchisq(RUS_RE_M$chisq, length(RUS_RE_M$n) - 1)

# Y vs. O cohort:
surv_RUS_RE_O <- filter(surv_RUS_RE, maternal_age != "6") # excluding M cohort
...
# Y vs. M cohort:
surv_Bp_M <- filter(surv_BpL1, maternal_age != "9") # excluding O cohort

Bp_M <- survdiff(Surv(surv_time, censor) ~ maternal_age, data = surv_Bp_M)
1 - pchisq(Bp_M$chisq, length(Bp_M$n) - 1)

# Y vs. O cohort:
surv_Bp_O <- filter(surv_BpL1, maternal_age != "6") # excluding M cohort
...
tiff("surv_curves.tiff", units="in", width=5, height=11, res=1200)
...
annotate_figure(surv_curves, left = textGrob("Survival probability", rot = 90, gp = gpar(fontsize = 16, fontface = 'bold')))
